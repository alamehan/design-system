...
function detectState() {
  ...
  /* A submodule is truly registered in git ONLY if git index tracks it as a gitlink.
     If .gitmodules mentions it but git index doesn't have it, it's an orphan/broken reference. */
...
function buildInstallPlan(cfg) {
  ...
  if (!st.subPopulated) {
    if (st.subRegistered) {
      /* Submodule is recorded in git index — populate it cleanly */
      ...
      /* Not in index (new install or orphan entry in .gitmodules).
         Using -f ensures git links cleanly even if an orphan entry or leftover git dir exists. */
...
async function execSteps(steps, ctx) {
  ...
  for (const s of steps) {
    ...
    try {
      ...
      if (s.cmd) {
        let r = sh(s.cmd);
        /* Self-healing: if 'git submodule update --init' fails due to missing pathspec in git index,
           automatically recover by force-adding the submodule instead of hard-failing. */
