...
function buildPromptSet(level) {
  ...
  return [
    ...
    p("stress", "Verify", "Gallery stress test", "Build a sample screen from the COMPLETE component gallery.",
      "Build a throwaway showcase page at pages/ds-stress.vue that exercises the COMPLETE component set from design-system/reference/gallery.html, mapped through .ds/bindings.md:\n- All 9 Input types (Basic, Search, SearchWithIcon, Dropdown, DatePicker, Password, PasswordShow, Number, Special) in Inactive, Active Single and Active Multi (value chips + clear) states.\n- Toast in all five semantic types (Success, Warning, Error, Info, Wait) - each with its shipped custom icon from design-system/src/assets/icons-custom, bold semantic title, subtitle, tonal action and circular dismiss.\n- Chip (Neutral/Colored/Gradient, MD and SM) side by side with StatusChip - same bold text ramp on both.\n- Tabs in all six types (Basic and Underline, each as TextOnly / IconAndText / IconOnly with aria-label).\n- DropdownMenu as a context menu: plain, with per-item leading icons, and with a search field (hasSearch).\n- RichTextEditor (toolbar: bold/italic/underline, lists, link) and RangeSlider (dual handle, min/max labels).\n- One compact DataTable inside a white card using at least 8 TableColumn types and 12 TableRow types, with pagination.\nAfter building, list any component you could not map 1:1 - that is a bindings gap to record in .ds/bindings.md, never a reason to hand-roll UI."),
  ];
}
